...
async function sendDropNowForSetting(client, setting) {
  try {
    ...
    const card = getRandomCardByProbability(PULL_PROBABILITIES);
    if (!card) {
      console.warn('Drop manager: no card chosen for drop.');
      return;
    }
...
export async function claimDrop(token, userId) {
  ...
  try {
    ...
    for (let i = 1; i < chain.length; i++) {
      ...
      if (e && (e.count || 0) > 0) ownedUpgrade = c;
    }
    const storedCard = ownedUpgrade || card;
